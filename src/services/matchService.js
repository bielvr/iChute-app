...
export async function getMatches({ leagueId, season, round, date }) {
  let query = supabase
    .from('matches')
    .select('*, home_team:teams!matches_home_team_id_fkey(id, name, url_logo), away_team:teams!matches_away_team_id_fkey(id, name, url_logo)')
    .eq('league_id', leagueId)
    .eq('season', season)
    .neq('status', 'postponed') // 👈 ADICIONADO: Ignora jogos adiados
    .order('date');
  ...
  if (date) { 
    ...
    query = query
      .gte('date', new Date(base.getTime() - 86400000).toISOString())
      .lte('date', new Date(base.getTime() + 86400000).toISOString()); 
  }
...
export async function getMatchCountsByDay({ leagueId, season, round }) { 
  let query = supabase
    .from('matches')
    .select('date')
    .eq('league_id', leagueId)
    .eq('season', season)
    .neq('status', 'postponed'); // 👈 ADICIONADO: Não conta jogos adiados na bolinha do calendário

  if (round) query = query.eq('round', round); 
  return throwIfError(await query); 
}
export async function getNearestMatch({ leagueId, season, direction }) { 
  let query = supabase
    .from('matches')
    .select('round, date')
    .eq('league_id', leagueId)
    .eq('season', season)
    .neq('status', 'postponed') // 👈 ADICIONADO
    .order('date', { ascending: direction === 'next' })
    .limit(1); 
